...
def ArtistList():  # currently not in use
    """
...
class ArtistProfile():
    ...
    def GetListUrls(self):
        ...
        try:
            ...
            print(f"{self.name} has {len(self.list_urls)} pages of song lyrics")  # TEXTING PRINT STATEMENT
        except:
            # Ensure no duplicated pages
            self.list_urls = list(set(self.list_urls))
            print(f"{self.name} only has {len(self.list_urls)} page of song lyrics")  # TEXTING PRINT STATEMENT
    ...
    def GetLyrics(self):
        ...
        """
        print(f"Extracting {len(self.song_lyrics)} song lyrics from {self.name}...")
