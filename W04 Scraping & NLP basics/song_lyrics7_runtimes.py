...
print("Initialised")  # TESTING PRINT STATEMENT
...
class ArtistProfile():
    ...
    def get_lyrics(self):
        ...
        """
        for song in tqdm(self.song_urls, desc = f"Extracting song lyrics for {self.name}..."):
    ...
    def spacy_encoder(self):
        ...
        """
        for song in tqdm(self.song_lyrics, desc = "Creating word tokens and cleaning lyrics"):
...
def scraper_workflow(self):
    ...
    for list_page in tqdm(self.list_urls, desc= f"Sending spiders to find lyrics of {self.name}"):
